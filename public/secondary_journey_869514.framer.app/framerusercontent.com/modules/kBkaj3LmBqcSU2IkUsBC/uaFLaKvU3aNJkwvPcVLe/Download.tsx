...
export default function Download(props) {
    const { styleOptions, hoverOptions, iconOptions, trackingId, onClick } =
        props
    const {
        backgroundColor,
        color,
    ...
    const handleClick = (e) => {
        ...
        onClick?.(e)
        // Do not prevent default; keep normal <a> behavior
    }
    ...
    return (
        <motion.a
            target="_blank"
            href={downloadURL}
            download={props.downloadName ?? "file.txt"}
            title={buttonTitle}
            style={{
                gap,
                fontSize: 16,
...
addPropertyControls(Download, {
    text: { title: "Text", type: ControlType.String, defaultValue: "Download" },
    srcType: {
...
const flexAlignSwitch = (fontControlStyles) => {
    if (!fontControlStyles?.textAlign) return "left"
    if (fontControlStyles.textAlign === "left") return "flex-start"
    if (fontControlStyles.textAlign === "right") return "flex-end"
    return "center"
}
